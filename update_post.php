...
if(Input::exists() && Token::check(Input::get('token'))){
	...
	if(isset($_FILES['media_url']) === true && empty($_FILES['media_url']['name']) !== true){
		...
		if($isFileOk){#upload file, remove old file and update DB
			...
			$post->update($post->data()->id, array('media_url' => $media_url, 'mime_type' => $file_type));#update the table
		}
	}
	...
	if(Input::get('desc')){
		$validateFields['desc'] = array('min' => 2, 'max' => 255);#add validation
		$updateFields['desc'] = Input::get('desc');
	}
	if(Input::get('tags')){
		$validateFields['tags'] = array('min' => 20, 'max' => 1023);#add validation
		$updateFields['tags'] = Input::get('tags');
	}
...
<div class="container" style="width:40%">
	<form action="" method="post" class="form-signin" role="form" enctype="multipart/form-data">
		...
		<h3>Description</h3>
		<input type="textArea" class="form-control" name="desc" id="desc" placeholder="<?php echo escape($post->data()->desc); ?>" autocomplete="off">
