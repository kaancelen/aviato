...
echo '<div class="thumbnail custom_thumbnail">';#thumbnail aç
	...
	if($current_post->user_id === $user->data()->id){ # if post owner is the logged in user
		echo "<a href='update_post.php?id={$current_post->id}'> <span class='glyphicon glyphicon-edit'>Mayn</span> </a>";
	}
	...
	if($rating_record->exists($user->data()->id, $current_post->id)){//if logged in user rate this post
